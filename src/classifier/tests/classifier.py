...
class TestHarness(Component):
    def construct(s, classifier, BIT_WIDTH=32, DECIMAL_PT=16, N_SAMPLES=8):
        ...
        s.cutoff_freq = stream.SourceRTL(mk_bits(BIT_WIDTH))
        s.cutoff_mag = stream.SourceRTL(mk_bits(BIT_WIDTH))
        s.sampling_freq = stream.SourceRTL(mk_bits(BIT_WIDTH))
        s.sink = stream.SinkRTL(mk_bits(1))
        s.classifier = classifier

        # Connect

        s.cutoff_freq.send //= s.classifier.cutoff_freq
        s.cutoff_mag.send //= s.classifier.cutoff_mag
        s.sampling_freq.send //= s.classifier.sampling_freq
...
def below_freq_mag():  # CUTOFF_FREQ = 8250 Hz, CUTOFF_MAG = 10
    ...
    return [
        ...
        655360,
        44000,
        get_fft_real(input_1),
        classifier(np.fft.fft(input_1), 8250, 10),
        get_fft_real(input_2),
        classifier(np.fft.fft(input_2), 8250, 10),
        get_fft_real(input_3),
        classifier(np.fft.fft(input_3), 8250, 10),
    ]
...
def above_freq_mag():  # CUTOFF_FREQ = 10000 Hz, CUTOFF_MAG = 20
    ...
    return [
        ...
        44000,
        get_fft_real(input_1),
        classifier(np.fft.fft(input_1), 10000, 20),
        get_fft_real(input_2),
        classifier(np.fft.fft(input_2), 10000, 20),
        get_fft_real(input_3),
        classifier(np.fft.fft(input_3), 10000, 20),
    ]
...
def one_true():  # CUTOFF_FREQ = 10000 Hz, CUTOFF_MAG = 40
    ...
    input_3 = [
        20 * np.sin(2 * np.pi * 3 * t / 16) + 20 * np.cos(2 * np.pi * 5 * t / 16)
    ...
    return [
        ...
        44000,
        get_fft_real(input_1),
        classifier(np.fft.fft(input_1), 10000, 40),
        get_fft_real(input_2),
        classifier(np.fft.fft(input_2), 10000, 40),
        get_fft_real(input_3),
        classifier(np.fft.fft(input_3), 10000, 40),
    ]
...
def random_4_decimal():  # BIT_WIDTH = 32, DECIMAL_PT = 4
    ...
    return [
        ...
        320,
        44000,
        get_fft_real(random_1),
        classifier(np.fft.fft(random_1), 10000, 200),
        get_fft_real(random_2),
        classifier(np.fft.fft(random_2), 10000, 200),
        get_fft_real(random_3),
        classifier(np.fft.fft(random_3), 10000, 200),
        get_fft_real(random_4),
        classifier(np.fft.fft(random_4), 10000, 200),
    ]
...
def sine_wave_below_mag():
    ...
    sine_wave = [
        generate_sine_wave_below_mag(f, sample_freq, cutoff_mag) for f in frequencies
    ]
    return [
        655360000,
        1310720,
        44000,
        get_fft_real(sine_wave[0]),
        classifier(np.fft.fft(sine_wave[0]), 10000, 200),
        get_fft_real(sine_wave[1]),
        classifier(np.fft.fft(sine_wave[1]), 10000, 200),
        get_fft_real(sine_wave[2]),
        classifier(np.fft.fft(sine_wave[2]), 10000, 200),
        get_fft_real(sine_wave[3]),
        classifier(np.fft.fft(sine_wave[3]), 10000, 200),
    ]
...
def random_sines():
    ...
    sine_wave = [
        a * np.sin(2 * np.pi * f * t) for a in rand_amplitudes for f in rand_frequencies
    ]
...
def test(test_params, cmdline_opts):
    ...
    input_data = [
        make_arr_fixed(test_params.BIT_WIDTH, test_params.DECIMAL_PT, x)
        for x in input_data
    ]
    ...
    th.set_param(
        "top.cutoff_freq.construct",
        msgs=[cutoff_freq, cutoff_freq, cutoff_freq],
        initial_delay=test_params.src_delay,
        interval_delay=test_params.src_delay,
    )
    ...
    th.set_param(
        "top.sampling_freq.construct",
        msgs=[sampling_freq, sampling_freq, sampling_freq],
        initial_delay=test_params.src_delay,
        interval_delay=test_params.src_delay,
    )
