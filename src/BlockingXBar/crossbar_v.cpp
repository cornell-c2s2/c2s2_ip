...
#include "obj_dir_crossbar/Vcrossbar.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_crossbar/Vcrossbar__Syms.h"
#include "svdpi.h"
#endif
...
Vcrossbar_t * create_model( const char *vcd_filename ) {
  ...
  m     = (Vcrossbar_t *) malloc( sizeof(Vcrossbar_t) );
  model = new Vcrossbar();
  ...
  m->control_rdy = &model->control_rdy;
  m->control_val = &model->control_val;
...
void destroy_model( Vcrossbar_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  Vcrossbar * model = (Vcrossbar *) m->model;
...
#if VLINETRACE
void trace( Vcrossbar_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_crossbar__v );
