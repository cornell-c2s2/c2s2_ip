...
from serializer-deserializer import SerializerDeserializer

# Creates a test harness class for the `SerializerDeserializer` module.
...
def create_model(n):
	model = SerializerDeserializer(n)

    # Create a harness wrapping our `SerializerDeserializer` module.
	return Harness(model, n)

@pytest.mark.parametrize('bitwidth, other_param', [
	(16, 1),
	(32, 1),
])
def test_simple(request, bitwidth, other_param):
	...
	run_sim(model, cmdline_opts={
		...
		'dump_vcd': f'{request.node.name}_{bitwidth}',
