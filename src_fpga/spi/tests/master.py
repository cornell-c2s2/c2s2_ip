...
from src_fpga.spi.master import SPIMaster


"""
Notes: figure out cs_addr and packet_size registers
"""
...
def t(
    ...
    Args:
        ...
        mosi: int - mosi value
        miso: int - miso value
        freq: list[int] - [val, rdy, msg] for the freq interface
    """
...
def test_8bit_val_often_freq0(
    cmdline_opts,
):  # two messages in a row - same CS - changing valrdy
    dut = SPIMaster(32, 2)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
    dut.apply(DefaultPassGroup(linetrace=True))
    reset(dut)
...
def test_8bit_2_msgs_no_read_freq0(
    cmdline_opts,
):  # 2 messages in a row, send both without reading the send data
    dut = SPIMaster(32, 2)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
    dut.apply(DefaultPassGroup(linetrace=True))
    reset(dut)
...
def test_8bit_2_msgs_no_read_freq7(
    cmdline_opts,
):  # 2 messages in a row, send both without reading the send data
    dut = SPIMaster(32, 2)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
