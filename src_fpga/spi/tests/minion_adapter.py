...
from src_fpga.spi.helpers.minion_adapter import SPIMinionAdapterWrapper


def test_basic(cmdline_opts):

    dut = SPIMinionAdapterWrapper(4, 1)
...
def test_queue_len2(cmdline_opts):
    ...
    dut = SPIMinionAdapterWrapper(4, 2)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
    dut.apply(DefaultPassGroup(linetrace=True))
...
def test_queue_len5(cmdline_opts):
    ...
    dut = SPIMinionAdapterWrapper(4, 5)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
    dut.apply(DefaultPassGroup(linetrace=True))
...
def test_more_bits(cmdline_opts):
    ...
    dut = SPIMinionAdapterWrapper(6, 1)
    dut = config_model_with_cmdline_opts(dut, cmdline_opts, duts=[])
    dut.apply(DefaultPassGroup(linetrace=True))
